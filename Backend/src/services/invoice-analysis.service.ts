...
import type { PoMatchStatus } from '@prisma/client';
...
const LARGE_INVOICE_THRESHOLD = 100_000;

function within(a: number, b: number, ratio: number, min: number): boolean {
  const tolerance = Math.max(min, Math.abs(b) * ratio);
  return Math.abs(a - b) <= tolerance;
}

interface HeaderMathResult {
  verified: boolean;
  itemSumVsSubtotal: { itemSum: number; subtotal: number | null; delta: number | null } | null;
  subtotalTaxDiscountVsTotal: { computed: number; stated: number | null; delta: number | null } | null;
}

function checkHeaderMath(
  ...
  const itemSumVsSubtotal =
    subtotalNum != null
      ? { itemSum, subtotal: subtotalNum, delta: itemSum - subtotalNum }
      : null;

  const computed = (subtotalNum ?? itemSum) + taxNum - discountNum;
  const subtotalTaxDiscountVsTotal =
    totalNum != null ? { computed, stated: totalNum, delta: computed - totalNum } : null;
...
function findDuplicateLineItems(
...
function assessFraudRisk(params: {
  ...
  headerMathVerified: boolean;
  duplicateLineCount: number;
  duplicateInvoiceNumberExists: boolean;
}): FraudAssessment {
...
async function matchPurchaseOrder(params: {
  ...
  const closeMatches = candidates.filter((po) =>
    within(params.totalAmount!, Number(po.totalAmount), PO_TOTAL_TOLERANCE_RATIO, 1)
  );
 ...
 */
export async function analyzeInvoice(invoiceId: number): Promise<void> {
  ...
  const headerMath = checkHeaderMath(
    invoice.items,
    invoice.subtotal,
    invoice.taxAmount,
    invoice.discount,
