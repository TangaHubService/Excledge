...
function zReportResource(branchId) {
 ...
 * but did not verify.
 */
exports.zReportJob = node_cron_1.default.schedule('55 23 * * *', async () => {
    if (!(0, rra_ebm_service_1.isEbmEnabled)()) {
        return;
    }
    try {
        const devices = await (0, vsdc_api_service_1.listActiveVsdcDevices)();
        let verified = 0;
        ...
        if (devices.length) {
            console.log(`[Z-report] daily close: devices=${devices.length} verified=${verified} savedUnverified=${savedUnverified} failed=${failed}`);
        }
    }
    catch (e) {
