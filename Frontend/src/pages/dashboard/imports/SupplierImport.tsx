...
export function SupplierImport({ onSuccess }: SupplierImportProps) {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      try {
        setPreviewing(true);
        const response = await apiClient.previewSupplierImport(selectedFile);
  ...
  const handleDownloadTemplate = async () => {
    try {
      const response = await apiClient.downloadSupplierTemplate();
      const blob = await response.blob();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = 'suppliers-import-template.xlsx';
  ...
  const handleConfirm = async () => {
    ...
    try {
      setConfirming(true);
      const response = await apiClient.confirmSupplierImport(previewData.importId);

      toast.success(
        `${response.imported} ${t('import.suppliersImported')}${response.errors > 0 ? ` (${response.errors} ${t('import.errors')})` : ''}`
      );
      ...
      if (response.errorFile) {
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = `supplier-import-errors-${previewData.importId}.xlsx`;
        document.body.appendChild(a);
        a.click();
        window.URL.revokeObjectURL(url);
        document.body.removeChild(a);
  ...
  const handleDownloadErrorFile = async () => {
    ...
    try {
      const response = await apiClient.downloadSupplierErrorFile(previewData.importId);
      const blob = await response.blob();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
  ...
  const supplierColumns = ['name', 'email', 'phone', 'address', 'contactPerson'];
